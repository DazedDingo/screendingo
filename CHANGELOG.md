...
- **Correct air dates on the episode list too (the Silo fix).** The title-detail Episodes section still showed TMDB's raw date — for Silo S3E10 that read "2026-09-03" while the episode actually dropped 02:00 UK on the 4th, sending the household hunting a day early. Episode rows now show the Trakt-resolved availability moment ("2026-09-04 ~08:00", your delay included) whenever Trakt knows the real time, falling back to the raw TMDB date otherwise. Resolved via a new `seasonAirMapProvider` family (one cached Trakt call per expanded season).
- **~10× fewer Trakt calls, faster Up Next.** Air times are now fetched one SEASON at a time (`TraktService.fetchSeasonFirstAired`, `GET /shows/{id}/seasons/{n}?extended=full` returns every episode's `first_aired`) instead of one call per episode, and cached on-device for 6 hours (`TraktSeasonAirCache`, prefs `wn_trakt_season_air`, entries pruned after 60 days). Steady-state Home opens now make zero Trakt calls. The per-episode `fetchEpisodeFirstAired` client method is gone; Cloud Functions still use their own per-episode fetch (unchanged this release).
- **The history entry point no longer vanishes.** When nothing airs in the next week the Up Next row used to collapse — taking the history button with it, exactly when you'd been away long enough to need it. `upNextProvider` now returns `UpNextData {episodes, recentCount}` (episodes available in the last 7 days, ≤10/show), and an empty row with `recentCount > 0` renders a slim "N recently aired ›" chip that opens the history screen. Old on-disk cache (bare list JSON) still parses.
- **Recently aired opens instantly.** The history provider is now a stale-while-revalidate stream (disk cache under `wn_upnext_history_cache` paints first, fresh data follows) — and the fresh pass is much faster anyway thanks to the season cache. Also fixed a real race found during the migration: the provider could settle on a stale empty result before the watch-entries stream had first emitted; it now waits for the first real emit, same guard the Up Next row uses.
- **Tests:** +23 (season fetch, season cache TTL/prune/corruption, recentCount, cache back-compat, history JSON roundtrip, seasonAirMapProvider fallback) → 1053 total. **Cost:** none — strictly fewer Trakt calls; TMDB unchanged.

## 0.12.31 (2026-09-01)

- **New "Recently aired" screen — the Up Next history.** If you don't open the app for a few days, an episode could slip out of the Up Next row's short window and you'd never know it dropped. The new screen lists every episode of your tracked shows that became available in the last **14 days**, grouped by day ("Today", "Yesterday", "Thu 28 Aug"), newest first, each row showing poster, S##E## · episode name, and the ~HH:mm availability time when known. Tapping a row deep-links to that episode on the title screen (and from there to Stremio). Pull down to refresh.
  - **Two ways in:** a small history icon on the Home "UP NEXT" row header, and the Profile → Insights "Up next" tile (now tappable, with a chevron). Note: the Home icon shows only while the Up Next row itself is visible; the Profile tile always works.
  - Same availability model as v0.12.30 (`availableAt` = Trakt `first_aired` + your configured delay, date-only fallback) via a new `upNextHistoryProvider` that reuses the shared tmdb→trakt id cache. Per show it reads `last_episode_to_air`; only when that's inside the window does it fetch the season and resolve each in-window episode (max 10/show). One show failing never blanks the screen. Known limitation: a prior season's episodes inside the window aren't fetched (only the season containing the last-aired episode).
  - `loadTraktIdCache`/`saveTraktIdCache` made public in `upnext_provider.dart` for the shared cache (no behaviour change).
  - **Tests:** +20 (provider window/sort/fallback/failure-isolation, day-label + grouping pure functions, screen widget tests). **Cost:** none — one extra TMDB season call per show with recent episodes, only while the screen is open; Trakt public API is free.

Also in this release window (no APK impact, shipped in `bc9c7e1`): `firebase-functions` 7.2.5 → 7.3.2 in Cloud Functions, and the release workflow now ignores `functions/**` so functions-only pushes can't overwrite an existing release.

## 0.12.30 (2026-08-26)

- **Up Next now uses the real broadcast time, not the US calendar date.** "Out today" was computed from TMDB's `next_episode_to_air.air_date` — a date-only string in the *network's* local date. For a UK household that's wrong by most of a day: Alone airs Wed 21:00 ET, which is Thu 02:00 UK, and torrents/streams land a few hours after that — so the row said "today" on Wednesday for an episode you couldn't find on Stremio until Thursday morning. Fix: every candidate episode is now resolved against Trakt's per-episode `first_aired` (a real UTC timestamp — public endpoints `GET /search/tmdb/{id}?type=show` for the id mapping, then `GET /shows/{traktId}/seasons/{s}/episodes/{e}?extended=full`; client-id header only, no OAuth, so it works whether or not Trakt is linked). `availableAt = first_aired + lag`, and the "today / tomorrow / in Nd" bucket is computed off `availableAt`'s calendar day, not the TMDB date. New rows carry `availableAt` + `airsAtUtc` (`hasAirTime`) and sort by `availableAt`; the old `airDate` field is kept for fallback. When Trakt has no time for an episode (or the call fails), behaviour is exactly the old date-only one.
  - **Availability delay setting.** Profile → Preferences → "Episode availability delay" (`upNextLagHoursProvider`, key `wn_upnext_lag_hours`, choices 0/2/4/6/8/12/24h, default **6h**). It's the gap between broadcast and the episode typically being streamable; 6h turns a 02:00 UK air into "Tomorrow ~08:00" when viewed the evening before and "Out now" from 08:00 on.
  - **Just-aired episodes no longer vanish early.** TMDB flips `next_episode_to_air` to the following week's episode as soon as the US date passes, so on Thursday morning the episode that had just become available disappeared from the row. Both `last_episode_to_air` and `next_episode_to_air` are now candidates per show; whichever is in-window with the earliest `availableAt` wins (`upNextCandidates` + `buildUpNextEpisode`, pure and unit-tested).
  - **Labels** now live in one place — `lib/utils/up_next_labels.dart` (`upNextRelativeLabel` for Home/widget, `upNextRelativeLabelCompact` for the Profile health line) — replacing three drifting copies. With a real air time: "Today ~08:00" → "Out now" once the time passes, "Tomorrow ~08:00"; without one the strings are unchanged ("Out today", "Tomorrow", "Aired yesterday", "Just aired", "In Nd"). HH:mm is local 24h.
  - **Cloud Functions get the same model** so the daily push and the 6-hourly widget refresh stop disagreeing with the app: new `functions/src/traktAirTime.ts` (`lookupTraktShowId`, `fetchEpisodeFirstAired`, `computeAvailability`, `candidateEpisodes`, `pickBestAvailability`, `relativeWhenLabel`, `UPNEXT_LAG_HOURS = 6`). `notifyNextEpisodeDaily` (09:00 UTC) now fires when `availableAt` falls on today UTC and words the push "is out now" / "is out today"; `refreshUpNextWidgetEvery6Hours` ranks by `availableAt` and emits the same label strings. Both bind the existing `TRAKT_CLIENT_ID` secret — no new secret to set. Server maths is UTC (it doesn't know the household's zone); the client recomputes locally on open.
  - Trakt tmdb→trakt id mapping is cached forever in SharedPreferences (`wn_trakt_show_ids`) so steady state is +1 Trakt call per show per refresh. Day arithmetic uses `DateTime.utc` on the client so DST-change days (23h/25h local days) can't truncate "tomorrow" to "today". Old on-disk Up Next cache entries without the new fields still parse (`availableAt` defaults to `airDate`).
  - **Tests**: +38 Dart (Trakt service public endpoints, provider Trakt path / last-beats-next / 404 fallback / id-cache / JSON back-compat, both label helpers every branch, lag setting) and +84 Jest (`traktAirTime.test.ts` + pipeline scenarios in both CF suites). **Cost**: none — Trakt public API is free/unmetered; no change to any paid service.
